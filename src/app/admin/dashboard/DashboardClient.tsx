...
export default function DashboardClient({ initialData }: DashboardClientProps) {
  ...
  return (
    <div className="min-h-screen bg-[#fbfaff] text-slate-800 flex flex-col md:flex-row relative overflow-hidden">
      {/* Background blobs */}
      <div className="absolute top-[10%] left-[5%] w-[40vw] h-[40vw] bg-indigo-500/5 rounded-full blur-[120px] pointer-events-none glow-blob" />
      <div className="absolute bottom-[10%] right-[5%] w-[35vw] h-[35vw] bg-primary/5 rounded-full blur-[110px] pointer-events-none glow-blob [animation-delay:4s]" />

      {/* Sidebar Panel */}
      <aside className="w-full md:w-64 glass border-b md:border-b-0 md:border-r border-white/60 p-6 flex flex-col justify-between gap-8 shrink-0 relative z-10">
          ...
          <div className="flex flex-row md:flex-col gap-1 overflow-x-auto pb-2 md:pb-0 scrollbar-none">
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${
                activeTab === 'overview' ? 'bg-primary text-white shadow-sm' : 'hover:bg-slate-200/50 text-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${
                activeTab === 'profile' ? 'bg-primary text-white shadow-sm' : 'hover:bg-slate-200/50 text-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${
                activeTab === 'projects' ? 'bg-primary text-white shadow-sm' : 'hover:bg-slate-200/50 text-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${
                activeTab === 'skills' ? 'bg-primary text-white shadow-sm' : 'hover:bg-slate-200/50 text-slate-700'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 justify-between transition-all cursor-pointer ${
                activeTab === 'inbox' ? 'bg-primary text-white shadow-sm' : 'hover:bg-slate-200/50 text-slate-700'
              }`}
            >
      ...
      <main className="flex-1 p-6 md:p-10 max-w-5xl overflow-y-auto relative z-10">
        ...
        {activeTab === 'overview' && (
          <div className="flex flex-col gap-8">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Overview Dashboard</h1>
              <p className="text-slate-500 mt-1 font-medium">Quick analysis of portfolio stats</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="glass p-6 rounded-2xl flex flex-col gap-2">
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Projects</h4>
                <p className="text-4xl font-black text-primary">{projects.length}</p>
              </div>
              <div className="glass p-6 rounded-2xl flex flex-col gap-2">
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Skills Categories</h4>
                <p className="text-4xl font-black text-cyan-500">{skills.length}</p>
              </div>
              <div className="glass p-6 rounded-2xl flex flex-col gap-2">
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Unread Messages</h4>
            ...
            <div className="glass p-6 rounded-2xl">
              ...
              {messages.length === 0 ? (
                <p className="text-sm font-semibold text-slate-500 py-6 text-center">Inbox is empty</p>
              ) : (
                <div className="space-y-4">
                  {messages.slice(0, 3).map((msg) => (
                    <div key={msg.id} className="border-b border-slate-200/50 pb-4 last:border-b-0 last:pb-0">
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <h4 className="font-bold text-sm text-slate-900">{msg.name} ({msg.email})</h4>
                          <p className="text-xs text-slate-500 font-medium mt-0.5">{msg.subject}</p>
                        </div>
                        <span className="text-[10px] font-semibold text-slate-500 uppercase">
                          {new Date(msg.createdAt).toLocaleDateString()}
                        </span>
                      </div>
                      <p className="text-sm font-medium text-slate-700 mt-2 line-clamp-2">
                        {msg.message}
                      </p>
                    </div>
                  ))}
        ...
        {activeTab === 'profile' && (
          <div className="flex flex-col gap-8">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Profile Details</h1>
              <p className="text-slate-500 mt-1 font-medium">Manage personal bio and social endpoints</p>
            </div>
            ...
            <form onSubmit={handleSaveProfile} className="glass p-6 md:p-8 rounded-3xl flex flex-col gap-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    required
                    className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:outline-none focus:border-primary transition-all text-sm font-medium text-slate-900"
                  />
                </div>
                <div className="flex flex-col gap-1.5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  required
                  rows={4}
                  className="px-4 py-3 rounded-xl border border-slate-200 bg-white/60 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:outline-none focus:border-primary transition-all text-sm font-medium text-slate-900 resize-none"
                />
              </div>
              ...
              <div className="border-t border-slate-200/60 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <label className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200/60 text-xs font-bold border border-slate-200 flex items-center gap-2 cursor-pointer transition-all text-slate-700">
                    {uploading === 'resume' ? (
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold text-sm shadow-[0_4px_12px_rgba(127,13,242,0.2)] transition-all cursor-pointer"
                >
        ...
        {activeTab === 'projects' && (
          <div className="flex flex-col gap-8">
            <div className="flex justify-between items-center gap-4">
              <div>
                <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Project Management</h1>
                <p className="text-slate-500 mt-1 font-medium">Add, edit, or remove portfolio items</p>
              </div>
              {!editingProject && (
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-[0_4px_12px_rgba(127,13,242,0.2)]"
                >
            ...
            {/* Editor form panel */}
            {editingProject !== null && (
              <form onSubmit={handleSaveProject} className="glass p-6 md:p-8 rounded-3xl flex flex-col gap-5 border border-primary/30 shadow-md">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-bold uppercase tracking-wider text-slate-500">Live Demo Link (Optional)</label>
                    <input
                      type="url"
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500">Technology Tags</label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={newTag}
                      onChange={(e) => setNewTag(e.target.value)}
                      className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white/60 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:outline-none focus:border-primary transition-all text-sm font-medium text-slate-900 flex-1"
                      placeholder="Add tag (e.g. React)"
                    />
                    <button
                      type="button"
                      onClick={addTag}
                      className="px-5 py-2.5 bg-slate-800 hover:bg-slate-950 text-white font-bold rounded-xl text-sm cursor-pointer transition-colors"
                    >
                      Add
                    </button>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-200/60 pt-5 mt-2">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      className="w-4 h-4 rounded text-primary focus:ring-primary border-slate-300 bg-white"
                    />
                ...
                <div className="flex justify-end gap-3 mt-4">
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-100 font-semibold text-sm cursor-pointer text-slate-700"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold text-sm shadow-[0_4px_12px_rgba(127,13,242,0.2)] cursor-pointer"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {projects.map((proj) => (
                <div key={proj.id} className="glass p-5 rounded-2xl border border-white/60 flex justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h4 className="font-bold text-slate-900 truncate">{proj.title}</h4>
                  ...
                  <div className="flex flex-col gap-2 justify-center shrink-0">
                    ...
                    <button
                      ...
                      className="p-2 bg-rose-500/10 hover:bg-rose-500 text-rose-500 hover:text-white rounded-xl cursor-pointer transition-colors"
        ...
        {activeTab === 'skills' && (
          <div className="flex flex-col gap-8">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Technical Skills</h1>
              <p className="text-slate-500 mt-1 font-medium">Manage skill tags and categories</p>
            </div>

            {/* Add Category Form */}
            <div className="glass p-6 rounded-2xl flex flex-col sm:flex-row gap-4 items-end">
              <div className="flex-1 flex flex-col gap-1.5 w-full">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500">New Category Name</label>
                <input
                  type="text"
              ...
              <button
                ...
                className="px-5 py-2.5 bg-primary hover:bg-primary-dark text-white font-bold rounded-xl text-sm cursor-pointer shrink-0 w-full sm:w-auto shadow-[0_4px_12px_rgba(127,13,242,0.2)]"
              >
                Add Category
              </button>
            </div>
                ...
                <div key={idx} className="glass p-6 rounded-2xl flex flex-col gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cat.list.map((skill, sIdx) => (
                      <span
                        key={sIdx}
                        className="px-2.5 py-1 text-xs font-semibold rounded bg-slate-100 text-slate-700 border border-slate-200/60 flex items-center gap-1.5"
                      >
                        <span>{skill}</span>
                        <button
                  ...
                  <div className="flex gap-2 mt-2">
                    <input
                      ...
                      className="px-3 py-1.5 rounded-lg border border-slate-200 bg-white/60 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:outline-none focus:border-primary transition-all text-xs font-medium flex-1 text-slate-900"
                      placeholder={`Add skill to ${cat.category}`}
                    />
                    <button
                      onClick={() => addSkillToCategory(cat.category)}
                      className="px-4 py-1.5 bg-slate-800 hover:bg-slate-955 text-white font-bold rounded-lg text-xs cursor-pointer transition-colors"
                    >
                      Add Skill
                    </button>
                  </div>
        ...
        {activeTab === 'inbox' && (
          <div className="flex flex-col gap-8">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Inbox Messages</h1>
              <p className="text-slate-500 mt-1 font-medium">View message submissions from contact form</p>
            </div>

            <div className="space-y-4">
              {messages.map((msg) => (
                <div key={msg.id} className="glass p-6 rounded-2xl border border-white/60 flex flex-col gap-4">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-600 mb-1">Subject: {msg.subject}</h4>
                    <p className="text-sm font-medium text-slate-700 leading-relaxed bg-white/50 p-4 rounded-xl border border-white/40 whitespace-pre-wrap">
                      {msg.message}
                    </p>
                  </div>
                </div>
              ...
              {messages.length === 0 && (
                <div className="glass p-12 rounded-2xl text-center">
                  <p className="text-slate-500 font-semibold">No messages received yet.</p>
                </div>
              )}
            </div>
          </div>
